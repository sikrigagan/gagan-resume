\documentclass[]{gagan-resume}


\begin{document}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     LAST UPDATED DATE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\lastupdated



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     TITLE NAME
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\namesection{Gagan}{Sikri}{ 
\urlstyle{same}\url{https://gagansikri.com} |
\href{mailto:me@gagansikri.com}{me@gagansikri.com} | +91-99915-59940}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     COLUMN ONE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{minipage}[t]{0.33\textwidth} 



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     SKILLS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Skills}
\subsection{Programming}
JavaScript, jQuery, VueJS, HTML5, CSS, PHP, Python
\subsection{Database}
MySQL, PostgreSQL, NoSQL, MongoDB, TaffyDB, Firebase
\subsection{Tools}
Atom, VSCode, Webstorm, Git, JIRA
\subsection{Experience With}
Data Visualization, UX Enhancement, App Architecture, Web Performance \& Design Patterns
\subsection{Design}
Sketch
\sectionsep



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EDUCATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Education} 

\subsection{Kurukshetra University}
\descript{B.Tech - Computer Science}
\location{2011 - 2015 | Kurukshetra, Haryana, IN \\ Cum. Percentage: 72.5\%}
\sectionsep



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     LINKS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Links} 
Github:// \href{https://github.com/sikrigagan}{\custombold{sikrigagan}} \\
Twitter://  \href{https://twitter.com/sikrigagan}{\custombold{@sikrigagan}} \\
LinkedIn://  \href{https://www.linkedin.com/in/sikrigagan}{\custombold{sikrigagan}} \\
Stack Overflow://  \href{https://stackoverflow.com/users/4129965/sikrigagan}{\custombold{sikrigagan}}
\sectionsep



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     WRITING
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Writing} 
\textbf{\href{https://dzone.com/users/2733735/sikrigagan.html}{DZone (6)}}\\
\textbf{\href{https://www.pluralsight.com/guides/author/sikrigagan}{Pluralsight Guides (2)}}\\
\textbf{\href{https://www.codementor.io/sikrigagan}{Codementor (1)}}\\
\textbf{\href{http://www.c-sharpcorner.com/members/gagan-sikri}{C\# Corner (11)}}\\
\sectionsep



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     LANGUAGES
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Languages}
English \\
Hindi \\
\sectionsep




%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     Passion
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Passion}
Programming \\
Products \\
Reading \\
Playing Badminton \\
\sectionsep



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     COLUMN TWO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{minipage} 
\hfill
\begin{minipage}[t]{0.65\textwidth}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EXPERIENCE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Experience}

\runsubsection{Sr. Technical Specialist}
\descript{| FusionCharts }
\location{April 2017 - Present | Bangalore, Karnataka, IN}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightemize}
\item Developing dashboards, POC’s and code samples for prospective and current clients using JavaScript, jQuery, PHP, MySQL, PostgreSQL, MongoDB, TaffyDB etc.
\item Working with product management and development team to continuously evolve the product based on user feedback, reported issues and new technology trends.
\item Handling development of FusionMaps.
\item Learning data visualisation principles and working with content marketing team for contribution of technical content as articles, tutorials etc on internal and external blogs.
\item Helping developers integrate product in their platforms and participation in external forums: \textbf{\href{https://stackoverflow.com/users/4129965/sikrigagan}{Stack Overflow}}, \textbf{\href{http://forum.fusioncharts.com/profile/67674-gagan-sikri/}{FusionCharts Forum}} 
\end{tightemize}
\sectionsep

\runsubsection{Technical Specialist}
\descript{| FusionCharts }
\location{September 2015 - March 2017 | Bangalore, Karnataka, IN}
\begin{tightemize}
\item Helped developers to integrate the product in their platforms by creating samples using different client side (JavaScript, jQuery) and server side (ASP.NET, PHP) technologies.
\item Handled development of FusionMaps.
\item Analysed complex issues of customers/prospects (seasoned developers or beginners) and provided solutions to them; and handled live queries as well.
\end{tightemize}
\sectionsep

\runsubsection{Technical Associate Intern}
\descript{| Holiday Plans}
\location{December 2014 - May 2015 | New Delhi, IN}
\begin{tightemize}
\item Worked with technology team for development of website.
\item Involved with marketing team for integration of marketing tools with website and developed internal moderation tools.
\end{tightemize}
\sectionsep



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     RECENT PROJECTS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Projects \& Experiments}
\runsubsection{LivSmart}
%\descript{| Head Undergrad Research}
\begin{tightemize}
\item Aadhaar Card based medical record and audit system with ability to provide collaboration for people with doctors, hospitals, blood banks, government and other international organisations.
\item Technologies Used: JavaScript, HTML5, CSS, ASP.NET, C\#, LINQ, Microsoft SQL Server 2012.
\end{tightemize}
\sectionsep

\runsubsection{Network Simulator}
%\descript{| Head Undergrad Research}
\begin{tightemize}
\item Deployed a small model to simulate data transmission over a 2-D plane of 1000 sq. meters and detect malicious node and packets virtually.
\item Technologies Used: ASP.NET, C\#, LINQ, Microsoft SQL Server 2012.
\end{tightemize}
\sectionsep



\end{minipage} 
\end{document}  \documentclass[]{article}
